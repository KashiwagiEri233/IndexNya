...
5. **技能系统** — 主 Agent 会自动识别请求并按需加载技能（如记忆卡片、费曼讲解、复习计划）；前端对话页可直接点选技能，输入主题后按技能说明执行。新增技能只需在 `backend/app/skills/` 下添加一个 Markdown 文件，无需改代码
...
8. **学习路径规划** — 根据学习目标和已有资料拆解学习步骤与先后关系
9. **多种答疑方式** — 支持文字、图片（上传题目/图表，由多模态模型直接识别解答），并可推荐 Bilibili 相关视频
10. **学习反馈** — 记录学习过程，查看阶段性进展和待改进的部分
11. **消息编辑 / 删除 / 引用** — 修改自己的提问、整轮删除（级联清理其探索卡片）、选中文本作为引用随问题发送
...
```bash
cp .env.example .env
# 推荐直接在前端「设置」页添加对话模型；服务端固定模型可选填 .env
```
...
后端 routers/       — chat / paths / tutoring / assessment / skills / practice（错题本） /
                       hierarchy（探索卡片 SSE）/ literature（文献）/ universe（思维宇宙）
后端 services/      — 资源生成、画像、路径、评估、分支对话、互动刷题、思维宇宙（嵌入/锚点/图）
后端 skills/        — Markdown 技能文件（backend/app/skills/*.md），主 Agent 按需加载执行
后端 llm/           — OpenAI 兼容接口封装，按配置切换模型；图片理解复用多模态能力
前端 pages/         — Chat / Profile / Practice（错题本） / Path / Dashboard /
...
3. **流式输出**：内容流结束后立即保存消息并发 `done`（前端光标即刻停止），验收 / 术语抽取 / 画像更新在后台并行执行，完成后回填消息 meta 并发出对应 SSE 事件。

互动刷题会话状态（进行到第几题、答对几题、每题答案与解析）持久化在消息 meta 的 `quiz_session` 字段中：同一对话里用户作答后，下一轮自动继续该会话，无需额外接口。

## 扩展技能

在 `backend/app/skills/` 下新建一个 Markdown 文件即可（支持 frontmatter）：
