...
4. **技能系统** — 主 Agent 会自动识别请求并按需加载技能（如记忆卡片、费曼讲解、复习计划）；前端对话页可直接点选技能，输入主题后按技能说明执行。新增技能只需在 `backend/app/skills/` 下添加一个 Markdown 文件，无需改代码
5. **层级对话（哪里不懂点哪里）** — 点击回答/文献中的术语，卡片在主线旁丝滑展开：
...
6. **思维宇宙** — 用自己的话总结概念，AI 评审认可后存入个人理解库；所有理解以 **3D 知识网络**呈现，讲解新概念时自动调用你已掌握的理解作为**知识锚点**
7. **学习路径规划** — 根据学习目标和已有资料拆解学习步骤与先后关系
8. **多种答疑方式** — 支持文字、图片（上传题目/图表，由多模态模型直接识别解答），并可推荐 Bilibili 相关视频
9. **学习反馈** — 记录学习过程，查看阶段性进展和待改进的部分
10. **消息编辑 / 删除 / 引用** — 修改自己的提问、整轮删除（级联清理其探索卡片）、选中文本作为引用随问题发送
...
后端 routers/       — chat / resources / paths / tutoring / assessment / skills /
