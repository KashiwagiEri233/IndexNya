  ...
  return (
    <div className="relative">
      <button
        ...
        className="inline-flex h-7 items-center gap-1.5 rounded-lg bg-claude-panel/70 px-2 text-[11px] font-bold text-claude-muted hover:bg-claude-accentSoft hover:text-claude-accent"
      ...
      {open && (
        <div className="absolute bottom-10 right-0 z-30 w-56 rounded-2xl border border-white bg-white p-2 shadow-island">
          <div className="flex items-center justify-between px-2 py-1.5">
            <div>
              <div className="text-sm font-extrabold text-claude-ink">推理强度</div>
              <div className="text-[11px] text-claude-muted">随请求透传给支持的模型接口</div>
            </div>
            ...
            {LEVELS.map((level) => {
              ...
              return (
                <button
                  ...
                  className={cn("flex w-full items-center gap-2 rounded-xl px-2 py-2 text-left hover:bg-claude-panel/70", active && "bg-claude-accentSoft/50")}
                >
                  <Check size={14} className={active ? "text-claude-accent" : "text-transparent"} />
                  <span className="min-w-0">
                    <span className="block text-xs font-bold text-claude-ink">{level.label}</span>
                    <span className="block truncate text-[10px] text-claude-muted">{level.hint}</span>
                  </span>
                </button>
              );
            })}
